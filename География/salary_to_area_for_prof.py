...
def salary_by_area_for_prof(csv):
...
def create_plot(csv_name):
    ...
    labelsize1 = 8
    rotation = 90
    salary_by_area = salary_by_area_for_prof(csv)
    ...
    plt.title(f'Уровень зарплат по городам для профеcсии Android-разработчик')
    plt.gca().invert_yaxis()
    plt.tick_params(axis='x', rotation=rotation, labelsize=labelsize1)
    plt.tick_params(axis='y', labelsize=6)
    plt.grid(axis='x')
    salary_by_area.columns = ['Город', 'Средний оклад']
    salary_by_area.to_csv(r'salary_by_area_for_prof.csv', index=False)
    plt.savefig('salary_by_area_for_prof')
    return plt
