...
def part_vac_by_area_for_prof(csv):
...
def create_plot(csv_name):
    ...
    vac_by_area = part_vac_by_area_for_prof(csv)
    ...
    plt.subplots_adjust(left=0.025, top=0.94, bottom=0)
    vac_by_area.columns = ['Город', 'Процент вакансий']
    vac_by_area.to_csv(r'part_vac_by_area_for_prof.csv', index=False)
    #plt.show()
    plt.savefig('part_vac_by_area_for_prof')
    return plt
